...
import { MovieCreate } from 'src/app/Models/MovieCreate';
...
export class MovieCreateComponent implements OnInit {
  ...
  gimmeGenre(id : number): PartialGenre {
    ...
    partial.fName = genre?.fName ?? ""
    return partial
  }
  genreId : number = 0
  addGenre() : void {
    if(this.genreId) {
      let partial = this.gimmeGenre(this.genreId)
  ...
  addMovie() : void {
    this.addSeries()
    this.addStudio()
    let movie = new MovieCreate()
    movie.title = this.movieForm.get("title")?.value ?? ""
    movie.description = this.movieForm.get("description")?.value ?? ""
    movie.dateOfRelease = new Date(this.movieForm.get("dateOfRelease")?.value ?? new Date)
   ...
   if(this.moviePhoto == null) return 
    movie.photo = this.moviePhoto
